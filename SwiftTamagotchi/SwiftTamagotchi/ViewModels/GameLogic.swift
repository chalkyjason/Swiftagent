...
class GameLogic: ObservableObject {
    ...
    private func startGameTimers() {
        gameTimer = Timer.scheduledTimer(withTimeInterval: 60.0, repeats: true) { [weak self] _ in
            self?.pet.updateStats()
        }

        saveTimer = Timer.scheduledTimer(withTimeInterval: 300.0, repeats: true) { [weak self] _ in
            self?.savePet()
        }
    }
